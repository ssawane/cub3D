...
/*   Updated: 2022/09/04 14:43:42 by ssawane          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3d.h"

void	texture_add(t_cub *cub)
{
	...
	else if (!ft_strncmp(cub->par[0], "DR", 2) && !cub->game->dr)
		cub->game->dr = ft_strdup(cub->par[1]);
	else if (!ft_strncmp(cub->par[0], "GN", 2) && !cub->game->gn)
		cub->game->gn = ft_strdup(cub->par[1]);
	else
		pars_free(cub, 1);
}
...
void	params(t_cub *cub)
{
	...
	if (!isempty(cub->line) && !ismap(cub->line))
	{
		if (!ft_strncmp(cub->line, "NO ", 3)
			...
			|| !ft_strncmp(cub->line, "DR ", 3)
			|| !ft_strncmp(cub->line, "GN ", 3))
			param_add(cub, 1);
